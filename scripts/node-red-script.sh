...
tgta14=14.20.0   # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl14=14.20.0   # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
tgta16=16.16.0   # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl16=16.16.0   # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
tgta18=18.7.0    # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
...
echo " "
echo "This script checks the version of node.js installed is 14 or greater. It will try to"
echo "install node 16 if none is found. It can optionally install node 12, 14, 16 or 18 LTS for you."
echo " "
...
case $yn in
    [Yy]* )
        ...
        if [[ "$nv" -lt 14 && "$nv" -ne 0  && "$nrv" != 1 ]]; then
            if [[ "$NODE_VERSION" == "" ]]; then
                ...
                echo "  "
                echo "  You can force an install of node 14, 16  or 18 by using the --node14, --node16 or --node18 parameter."
        ...
        if [[ "$APTOK" == "false" ]]; then
            ...
            CHAR="-"
            echo -ne "  Node option not specified           :   --node14, --node16, or --node18\n"
            echo -ne "  Leave existing Node.js              :"
        else
